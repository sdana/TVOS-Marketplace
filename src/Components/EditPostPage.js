                                ...
                                <form onSubmit={(e) => {
                                    ...
                                    <Grid container xs={12} direction="row" justify="flex-start">
                                        {this.state.post.photo.map((photo, index) => {
                                            return <div style={{ margin: "5px 15px" }}><div className="tooltip"><span className="tooltiptext" id={index} onClick={e => this.removePostedPhoto(e)}>Click To Delete</span><img src={`${photo}`} style={{maxHeight:300}} /></div></div>})}
                                    </Grid>
